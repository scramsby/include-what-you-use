...
Since we're not running using the Microsoft Visual C++ compiler (IWYU is built on top of Clang), we have no way of telling if foo is a necessary `#include` or not.
